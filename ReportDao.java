...
public class ReportDao extends BaseDao {
    ...
    public void reportInstanceDataHorizontal(int instanceId, final ReportDataStreamHandler handler) {
        ...
        // Declare an ArrayList to store ReportDataValue objects
        List<ReportDataValue> rdvs = new ArrayList<>();

        for (final ReportPointInfo point : pointInfos) {
            handler.startPoint(point);

            // Move ReportDataValue creation inside the loop
            ...
            ejt.query(REPORT_INSTANCE_DATA_SELECT + "where rd.reportInstancePointId=? order by rd.ts",
                    new Object[] { point.getReportPointId() }, new RowCallbackHandler() {
                        public void processRow(ResultSet rs) throws SQLException {
                            ...
                            // Add the ReportDataValue object to the ArrayList
                            rdvs.add(rdv);
        ...
        // Pass the ArrayList to the handler for further processing
        handler.done(rdvs);
    }
