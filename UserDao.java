...
public class UserDao extends BaseDao {
    ...
    void updateUser(User user) throws DataIntegrityViolationException{
        String phone = user.getPhone();
        String homeUrl = user.getHomeUrl();
        //creating storing data in usr object
           Object[] usr = {
                   user.getUsername(),
                   user.getPassword(),
                   user.getEmail(),
                   phone, // can be null
                   // boolean data stored as 't' or 'f' char
            ...
            // attempting to update user in users table with 'usr' object
            try {
                ejt.update(USER_UPDATE, usr);
            }
            // database cannot accept data
            catch (DataIntegrityViolationException e) {
                // user will not be updated
                log.warn("Error", new Exception("Please verify all user fields have been entered correctly"));
