...
public class ReportDao extends BaseDao {
    ...
    public void reportInstanceData(int instanceId, final ReportDataStreamHandler handler) {
        ...
        for (final ReportPointInfo point : pointInfos) {
            ...
            ejt.query(REPORT_INSTANCE_DATA_SELECT + "where rd.reportInstancePointId=? order by rd.ts",
                    new Object[] { point.getReportPointId() },
                    new RowCallbackHandler() {
                        public void processRow(ResultSet rs) throws SQLException {
                            switch (dataType) {
    ...
    public void reportInstanceDataHorizontal(int instanceId, final ReportDataStreamHandler handler) {
        // Retrieve point information.
        List<ReportPointInfo> pointInfos = query(REPORT_INSTANCE_POINT_SELECT + "where reportInstanceId=?",
                new Object[] { instanceId }, new GenericRowMapper<ReportPointInfo>() {
                    public ReportPointInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        HashMap<String, ArrayList<ReportPointInfo>> entries = new HashMap<>();
        for (final ReportPointInfo point : pointInfos) {
            final String pointName = point.getExtendedName();

            if (!entries.containsKey(pointName)) {
                entries.put(pointName, new ArrayList<>(List.of(point)));
            } else {
                entries.get(pointName).add(point);
            }
        }

        // start here
        var things = entries.entrySet()
                .stream()
        ...
        int col_no = -1;
        int row_no = 0;
        String prev_sensor = null;
        List<SensorRow> pt_list = new ArrayList<>();
        for (final Map.Entry<String, ReportPointInfo> thing : things) {
            if (!Objects.equals(thing.getKey(), prev_sensor)) {
                prev_sensor = thing.getKey();
                row_no = 0;
                col_no++;
            }
            SensorRow sensor = new SensorRow(row_no, col_no, thing.getValue());
            pt_list.add(sensor);
            row_no++;
        }

        SensorRow[] pt_list_arr = new SensorRow[pt_list.size()];
        for(int i = 0; i < pt_list.size(); i++) pt_list_arr[i] = pt_list.get(i);
        sort_points(pt_list_arr);
        List<ReportPointInfo> rpi_list = new ArrayList<>();
        for (SensorRow sensorRow : pt_list_arr) {
            rpi_list.add(sensorRow.rpi);
        }

        final ReportDataValue rdv = new ReportDataValue();
        for (final ReportPointInfo point : rpi_list) {
            handler.startPoint(point);
            ...
            ejt.query(REPORT_INSTANCE_DATA_SELECT + "where rd.reportInstancePointId=? order by rd.ts",
                    ...
                    new RowCallbackHandler() {
                        public void processRow(ResultSet rs) throws SQLException {
                            switch (dataType) {
                                case (DataTypes.NUMERIC) -> rdv.setValue(new NumericValue(rs.getDouble(1)));
                                case (DataTypes.BINARY) -> rdv.setValue(new BinaryValue(rs.getDouble(1) == 1));
                                case (DataTypes.MULTISTATE) -> rdv.setValue(new MultistateValue(rs.getInt(1)));
                                case (DataTypes.ALPHANUMERIC) -> {
                                    rdv.setValue(new AlphanumericValue(rs.getString(2)));
                                    if (rs.wasNull())
                                        rdv.setValue(new AlphanumericValue(rs.getString(3)));
    ...
    static class SensorRow {
        public final int y, x;
        public final ReportPointInfo rpi;

        public SensorRow(int y, int x, ReportPointInfo rpi) {
            this.y = y;
            this.x = x;
            this.rpi = rpi;
        }
    ...
    static void sort_points(SensorRow[] a) {
